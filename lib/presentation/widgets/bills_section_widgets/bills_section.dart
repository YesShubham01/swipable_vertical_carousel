...
/// a swipable vertical carousel driven by backend data.
///
///   Handles three states:
/// - Loading → Shimmer placeholder
/// - Error → Error message
/// - Success → Render header + carousel
...
class BillsSectionState extends State<BillsSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final listOfChildCards = sectionData.templateProperties.childList;
    final billsCount =
        int.tryParse(
          sectionBody.billsCount.toString(),
        )?.clamp(0, listOfChildCards.length) ??
        1;
